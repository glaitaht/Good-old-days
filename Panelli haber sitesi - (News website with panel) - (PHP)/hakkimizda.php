  ...
  <div class="container-fluid" height="100%">
    ...
    <div class="collapse navbar-collapse" id="myNavbar" height="100%" style="margin-top:1.5em">
      <ul class="nav navbar-nav navbar-right" style="margin-right:6%;">
        ...
        <li><a href="hakkimizda.php" class="active">Yazarlarımız</a></li>
...
<div class="container-fluid bg-3 text-center" >    
  <br>
<?php // hakkımızda bölümü için
			$sql="select * from yazarlar ";// bütün yazarlar sorgu
			$result = $conn->query($sql);// sorgu çalıştı
			if ($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					echo "
					<div class='card' style='width: 18rem;'>
					  <img class='card-img-top img-responsive' src='img/".$row['resimDir']."' alt='Card image cap'>
					  <div class='card-body'>
						<h5 class='card-title'>".$row['yazarAd']." ".$row['yazarSoyad']."</h5>
						<p class='card-text'>".$row['hakkinda']."</p>
						<a href='yazar.php?id=".$row['yazarID']."' class='btn btn-primary'>Bütün haberleri</a>
					  </div>
					</div>
					";//her yazar için yazdı.
				}
			}
?>
</div>
