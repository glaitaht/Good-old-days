...
<?php 
	if(isset($_GET['id']) || !empty($_GET['id'])){//eğer url ile id taşındıysa
			echo"
			  <h2>Yazarın bütün haberleri:</h2><br>
			  ";
			$sql="select * from haberler where yazarID=".$_GET['id']." order by hID DESC";//yazarın bütün haberlerini çekecek
			$result = $conn->query($sql);//çalıştırdı
			$hbr=0;//haber sayısını tuttu
			if ($result->num_rows > 0) {
				while($row = $result->fetch_assoc()) {
					if($hbr % 4 == 0){//eğer 4 se satır başı
						echo"<div class='row'>";
					}
					echo "
					...
					";//yazarın haberini yazdırdı.
					if($hbr % 4== 3){//eğer 3 se satır sonu
						echo "</div><br /><br />";
					}
					$hbr++;
				}
